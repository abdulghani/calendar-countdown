...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-2xl flex-col justify-center gap-4 p-6 text-white">
      <h1 className="text-3xl font-semibold">{message}</h1>
      <p className="text-white/60">{details}</p>
      {stack && (
        <pre className="w-full overflow-x-auto rounded-lg bg-white/5 p-4 text-xs text-white/50">
          <code>{stack}</code>
        </pre>
      )}
